...
void Extruder::makeLoftObject()
{
	...
	// color code all sides... for Spokane (3 sides)

	for (int k = 0; k < fUpperLimit; k++) {
		//material_count = 0;
		int i = 0;
		for (i = 0; i < numShapeVertices-1; i++) 
		{
			int start = i + k * numShapeVertices;
			fLoftFaces->coordIndex.set1Value(count++, start);
			fLoftFaces->coordIndex.set1Value(count++, start + 1);
			fLoftFaces->coordIndex.set1Value(count++, start + numShapeVertices);
			fLoftFaces->coordIndex.set1Value(count++, -1);

			fLoftFaces->coordIndex.set1Value(count++, start + 1);
			fLoftFaces->coordIndex.set1Value(count++, start + numShapeVertices + 1);
			fLoftFaces->coordIndex.set1Value(count++, start + numShapeVertices);
			fLoftFaces->coordIndex.set1Value(count++, -1);
