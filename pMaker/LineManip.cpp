...
SbColor LineManip2::pickColor;
SbColor LineManip2::locateColor;
...
LineManip2::LineManip2()
{
    ...
    locateColor = SbColor(0.5, 0.5, 0.5);
    pickColor = SbColor(1, 1, 1);
    ...
    pset = new SoPointSet;
	 SoDrawStyle* psetStyle = new SoDrawStyle;
	 psetStyle->pointSize.setValue(9);
	 addChild(psetStyle);
	 //pset->numPoints.setValue(SoPointSet::SO_POINT_SET_USE_REST_OF_POINTS);
     addChild(pset);
    ...
    addChild(drawStyle);
	 ...
	 SoMaterialBinding* lineMatBinding = new SoMaterialBinding;
	 //lineMatBinding->value.setValue(SoMaterialBinding::PER_PART);
	 lineMatBinding->value.setValue(SoMaterialBinding::OVERALL);
	 lineMaterial = new SoMaterial;
	 lineMaterial->diffuseColor.setValue(1, .6, 0);
	 //lineMaterial->diffuseColor.set1Value(1,0,1,0);
	 //lineMaterial->diffuseColor.set1Value(2,0,0,1);
    ...
    lset = new SoLineSet;
	//lset->numVertices.setValue(SoLineSet::SO_LINE_SET_USE_REST_OF_VERTICES);
    addChild(lset);
...
{
    ////// test 
    //int num = coord->point.getNum();
    //if (num>2){
    //    if(coord->point[0] == coord->point[num-1])
   //         coord->point.deleteValues(num-2, -1);
    //}
     ////// endtest
...
LineManip2::handleEvent(SoHandleEventAction *ha)
{
   ...
   //else if (SO_KEY_PRESS_EVENT(e, SoKeyboardEvent::BACKSPACE) ||
	//	 SO_KEY_PRESS_EVENT(e, SoKeyboardEvent::DELETE)) 
	else if ((SoKeyboardEvent::isKeyPressEvent(e, SoKeyboardEvent::BACKSPACE))) // ||
		//(SoKeyboardEvent::isKeyPressEvent(e, SoKeyboardEvent::DELETE)))
	{	
		SbBool test = remove();
		if (TRUE == test)
			ha->setHandled();
...
LineManip2::whichPart()
{
	...
	//    v1 = current_position - coord->point[0];
	 //   v2 = current_position - coord->point[n-1];
	    //
...
LineManip2::dragStart()
{
	...
	break;
	*/
	current_coord = coord->point.getNum();
	coord->point.set1Value(current_coord, current_position);
	myHandleEventAction->setGrabber(this);
	hilightVertex(coord->point[current_coord], pickColor);
	break;
...
LineManip2::remove()
{
    if (coord == NULL)
	return FALSE;
	 if (coord->point.getNum() == 1) return FALSE;  //////////// pauls addition
