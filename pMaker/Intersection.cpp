...
// PS -- I changed this code to use Inventor classes....
...
float 
Intersection::_dot(SbVec2f vA, SbVec2f vB) { 
      float dot = (vA[0] * vB[0]) + (vA[1] * vB[1]);
	  //TRACE("dot = %f\n", dot);
      return dot;
   }

float 
Intersection::_perp(SbVec2f vA, SbVec2f vB) { 
      float perp = (vA[0] * vB[1]) - (vA[1] * vB[0]);
	  // TRACE("perp = %f\n", perp);
      return perp;   // 0 = perpendicular
   }
...
int
Intersection::intersect2D_2Segments(SbVec2f * S1, SbVec2f * S2, SbVec2f & I0, SbVec2f & I1)
{
//	TRACE("s1 = [%f, %f] [%f, %f]\n", S1[0][0], S1[0][1], S1[1][0], S1[1][1]);
//	TRACE("s2 = [%f, %f] [%f, %f]\n", S2[0][0], S2[0][1], S2[1][0], S2[1][1]);
	SbVec2f    u = S1[1] - S1[0];  // derive origin-based vector of segment
	SbVec2f    v = S2[1] - S2[0];  // vector of 2nd segment
	SbVec2f    w = S1[0] - S2[0];  // vector between begin-points of each segment
	float      D = _perp(u, v);

	// test if  they are parallel (includes either being a point)
	if (fabs(D) < SMALL_NUM) {           // S1 and S2 are parallel
		if (_perp(u, w) != 0 || _perp(v, w) != 0) {
		...
		if (du == 0) {                       // S1 is a single point
			if (inSegment(S1[0], S2) == 0)   // but is not in S2
				return 0;
			I0 = S1[0];
			return 1;
		}
		if (dv == 0) {                     // S2 a single point
			if (inSegment(S2[0], S1) == 0)  // but is not in S1
				return 0;
			I0 = S2[0];
			return 1;
		}
	...
	float     sI = _perp(v, w) / D;
	if (sI < 0 || sI > 1)                // no intersect with S1
		return 0;

	// get the intersect parameter for S2
	float     tI = _perp(u, w) / D;
	if (tI < 0 || tI > 1)                // no intersect with S2
		return 0;

	I0 = S1[0] + sI * u;                // compute S1 intersect point
	return 1;
}
//===================================================================
...
//            0 = P is  not inside S
int
Intersection::inSegment(SbVec2f P, SbVec2f * S)
{
	if (S[0][0] != S[1][0]) {    // S is not  vertical
