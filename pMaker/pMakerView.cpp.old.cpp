...
TRACE("made it here -- on_initial_update\n");
    ...
    midlineLineSet->ref();
#endif
	threeDCoords = new SoCoordinate3;
	threeDCoords->ref();
	...
	sectionCoords->point.setValue(SbVec3f(0,0,0));
    sectionCoords->point.set1Value(1,SbVec3f(1,0,0));
	...
	//loftFaces = new SoIndexedFaceSet;

	//loftMaterial = new SoMaterial;
...
void CpMakerView::OnShowDialog() // call this to regenerate the extrusion
{
    ...
    if((vScaleCoords->point[1] - vScaleCoords->point[0]).length() - 1.0 < 0.001) {
        branch_sep = fExtruder->extrude(this->sectionCoords,this->threeDCoords, this->hScaleCoords, this->hScaleCoords, 1.0, 0.0, false );
    } else {
        branch_sep = fExtruder->extrude(this->sectionCoords,this->threeDCoords, this->hScaleCoords, this->vScaleCoords, 1.0, 0.0, false );
    }
...
void CpMakerView::OnLoadConstructionData() 
{
    ...
    if((vScaleCoords->point[0] - vScaleCoords->point[vScaleCoords->point.getNum() -1]).length() - 1.0 < 0.001) {
        loftRoot->addChild(fExtruder->extrude(sectionCoords, threeDCoords, hScaleCoords, hScaleCoords, 1.0, 0.0, false));
    } else {
        loftRoot->addChild(fExtruder->extrude(sectionCoords, threeDCoords, hScaleCoords, vScaleCoords, 1.0, 0.0, false));
    }
}
...
void CpMakerView::OnFlatten() //quadrilaterals
{
	...
	int numPathCoords = threeDCoords->point.getNum();  // !!! just now...
...
/*
void
CpMakerView::mouseClick(void *viewer, SoEventCallback *eventCB)
{
	...
	if (ev->isOfType(SoLocation2Event::getClassTypeId()))
	{
		...
		if(dpath == NULL) 
      {
       ...
       SoPerspectiveCamera *camera = 
           (SoPerspectiveCamera *)theView->IvfGetActiveCamera();
       SoWinExaminerViewer * eViewer = (SoWinExaminerViewer *)theView->m_pComponent;
...
// maryland FINAL design parameters: 
float sCenterlineScale[MAX_LEVELS] = { .9, .9,  .9,     1.1,   1.1,   1.2,   1.4,   0,   0  };       // ('path scale')
float sThickness[MAX_LEVELS] =       {.25, .25, .1875, .1875, .1875, .1875, .1875, .1875, .1875 };   // steel thicknesses at each level
...
void CpMakerView::OnChangeLoftScale()
{
    ...
    loftRoot->removeAllChildren();
    if(((vScaleCoords->point[1] - vScaleCoords->point[0]).length() - 1.0) < 0.001) {
        loftRoot->addChild(fExtruder->extrude(scaled_section_coords, scaled_centerline_coords, hScaleCoords, hScaleCoords, 1.0, thickness, true));
    } else {
        loftRoot->addChild(fExtruder->extrude(scaled_section_coords, scaled_centerline_coords, hScaleCoords, vScaleCoords, 1.0, thickness, true));
    }
}
...
void CpMakerView::OnFlattenAllLevels()
{
    ...
    float section_scale;
    float thickness;
    int num_three_d_coords = threeDCoords->point.getNum();
    int num_section_coords = sectionCoords->point.getNum();
    int num_sides = num_section_coords - 1;

    for(int i = 0; i < fNumLevels; i++)
    {
        section_scale = (pow(fScaleFactor, i));
        centerline_scale = sCenterlineScale[i];
        thickness = sThickness[i];
        ...
        for(int i = 0; i < num_section_coords; i++){
            SbVec3f dst;
            section_scale_matrix.multVecMatrix(sectionCoords->point[i], dst);
            scaled_section_coords->point.set1Value(i, dst);
        }

        for(int i = 0; i < num_three_d_coords; i++){
            SbVec3f dst;
            SbVec3f dst2;
            section_scale_matrix.multVecMatrix(threeDCoords->point[i], dst);
            centerline_scale_matrix.multVecMatrix(dst, dst2);
        ...
        SoSeparator * extrusion = (fExtruder->extrude(scaled_section_coords, scaled_centerline_coords, hScaleCoords, vScaleCoords, 1.0, thickness, false));
        extrusion->ref();
        SoCoordinate3 * coords = (SoCoordinate3 *)findChildOfType(extrusion, SoCoordinate3::getClassTypeId());  
        SoCoordinate3 * loft_coords = new SoCoordinate3;
        loft_coords->ref();
        
        for(int i = 0; i < coords->point.getNum(); i++)
            loft_coords->point.set1Value(i, coords->point[i]);

        char level_string[20];
        sprintf(level_string, "_%d", i);
        CString level_str = level_string;

        CString iv_filename = filename_prefix;
        iv_filename += level_str;
        iv_filename += "_e.iv";

        CString dxf_filename = filename_prefix;
        dxf_filename += level_str;
        dxf_filename += "flat.dxf";  
        
        Flattener::flatten_polylines(loft_coords, num_sides, num_three_d_coords, dxf_filename);
        
        SoWriteAction wa;

        wa.getOutput()->openFile(iv_filename);
        extrusion->ref();
        wa.apply(extrusion);
        extrusion->unref();
        wa.getOutput()->closeFile();
